...
class LibraryViewModel(
    ...
    companion object {
        val TYPE_KEY = CreationExtras.Key<LibraryType>()

        val Factory = viewModelFactory {
            initializer {
                LibraryViewModel(type = get(TYPE_KEY) ?: LibraryType.All)
            }
        }
    }
        ...
        return combine(
            ...
            if (!isNovelBackfillDone) {
                ...
                if (toUpdate.isNotEmpty()) {
                    viewModelScope.launchIO { updateManga.awaitAll(toUpdate) }
                }
            }
    ...
    fun reloadLibraryFromDB() {
        viewModelScope.launchIO {
            logcat { "LibraryViewModel: User requested library reload" }
            val library = getLibraryManga.refreshForced()
            logcat { "LibraryViewModel: Library reloaded with ${library.size} items" }
        }
    }
